...
Z=zeros(70,all_num);
z=z(:);
left_b=zeros(100,100,70);
left_t=zeros(100,100,70);
left_t_2=zeros(100,100,70);
for i=len+1:100+len
    for j=len+1:100+len
...
%        A5=GAMMA((1+4*all_num):(5*all_num));
       ...
       %柴麻r_b=sqrt(sum|1->5 ||(x_i-A_b*beta_i)||2)
        %柴麻r_t=sqrt(sum|1->5 ||(x_i-A_t*alpha_i)||2)
...
%        Dx=r_b/r_t;
       ...
       left_b(i-len,j-len,:)=SS_b-Dict_b*alpha1;
       left_t(i-len,j-len,:)=SS_b-Dict_t*beta1;
%        SS_t= left_t(i-len,j-len,:);SS_t=SS_t(:);
...
imshow(P_compare);
subplot(2,2,3);
imshow(uint8(floor(left_b(:,:,20))));
subplot(2,2,4);
imshow(uint8(floor(left_t(:,:,20))));